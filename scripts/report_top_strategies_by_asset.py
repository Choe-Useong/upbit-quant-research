...
DEFAULT_OUTPUT_MD = ROOT_DIR / "data" / "strategy_db" / "reports" / "top_strategies_by_asset.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build an asset-by-asset top strategy report from strategy_results_master.csv."
    )
    parser.add_argument(
        "--input-csv",
    ...
    parser.add_argument("--top-n", type=int, default=5, help="Top N rows per asset")
    parser.add_argument(
        "--sort-by",
...
def _load_and_filter(args: argparse.Namespace) -> pd.DataFrame:
    ...
    if "asset_scope" not in frame.columns:
        raise SystemExit("asset_scope column not found.")
    return frame


def _select_top_by_asset(frame: pd.DataFrame, sort_by: str, ascending: bool, top_n: int) -> pd.DataFrame:
    filtered = frame[frame["asset_scope"].fillna("").astype(str) != ""].copy()
    ordered = filtered.sort_values(by=["asset_scope", sort_by], ascending=[True, ascending], na_position="last")
    return ordered.groupby("asset_scope", dropna=False, group_keys=False).head(top_n).reset_index(drop=True)


def _write_markdown_report(frame: pd.DataFrame, output_md: Path, columns: list[str], top_n: int, sort_by: str) -> None:
    output_md.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    lines.append("# Top Strategies By Asset")
    lines.append("")
    lines.append(f"- Top N: `{top_n}`")
    lines.append(f"- Sort By: `{sort_by}`")
    lines.append("")
    for asset_scope, asset_frame in frame.groupby("asset_scope", dropna=False):
        asset_label = str(asset_scope) if pd.notna(asset_scope) and str(asset_scope) else "UNKNOWN"
        lines.append(f"## {asset_label}")
        lines.append("")
        asset_table = _frame_to_markdown(asset_frame[columns])
        lines.append(asset_table)
        lines.append("")
...
def main() -> None:
    ...
    result = _select_top_by_asset(frame, args.sort_by, args.ascending, args.top_n)
